...
unsigned long ms_mqtt_lastsent = 0;   // elapsed millisecond ts since last mqtt publish
...
void update_main_page_sensor_table () {
  ...
  a += ", max-=";
  a += analog_soil_max;
  s += client.generate_html_table_row ("td", "analog value (soil)", String(a)); 
  s += "</table>";
...
void loop() {
  ...
  if (millis() - ms_mqtt_lastsent >= 60E3 or ms_mqtt_lastsent == 0){
     ...
     if (not isnan(soil_perc) and analog_value != 0 ) { 
